...
export class TodoItemComponent implements OnInit {
  ...
  constructor(
    public todoService: todoService /*private toasterService: ToastrService*/
  ) {}
  ...
  setStatus(status) {
    console.log("ee",status, this.todoInput ,status);
    this.todoInput.status = status;
    this.todoService.updateTodo(status, this.todoInput.id).subscribe(
      (data) => {},
      (err) => {}
    );
    console.log("aa", status,this.todoInput ,status);
  }
  toggleClass() {
  ...
  buttonClass(status) {
    if (this.todoInput.status.valueOf() == "status") {
      ...
      return {
        "badge badge-info mr-1": true,
        "badge badge-pill badge-primary": false,
      };
    } else {
